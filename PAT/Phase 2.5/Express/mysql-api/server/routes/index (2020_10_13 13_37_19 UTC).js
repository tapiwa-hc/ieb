...
router.put('/edit', verifyStaff, async (req, res) => {
  ...
  try {
    let results = await db.editNotice(req.body.id, req.body.title, req.body.description, req.user.rights);
    res.end();
  } catch (e) {
    console.log(e);
    res.sendStatus(500);
...
router.post('/new', verifyStaff, async (req, res) => {
  ...
  try {
    let results = await db.newNotice(req.body.uID, req.body.display_date, req.body.gID, req.body.title, req.body.description);
...
router.get('/feed/:id', verifyMobile, async (req, res) => {
...
router.post('/password', verifyPupil, async (req, res) => {
  ...
  try {
    let results = await db.changePassword(req.user.username, req.body.oldPass, req.body.newPass);
    res.end();
  } catch (e) {
    console.log(e);
    res.status(401).send(e);
