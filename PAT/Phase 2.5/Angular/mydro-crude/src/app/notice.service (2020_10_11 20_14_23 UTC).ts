...
import { PendingNotice, TabularNotice, Feed, NewNotice, printList } from './interfaces/Notices';
...
export class NoticeService {
  ...
  getMyNoticesMobile(): Observable<Feed[]> {
    return this.http.get<Feed[]>(`${this.dbUrl}/mobile/feed`).pipe(
      catchError(this.handleError<Feed[]>('getMyNoticesMobile', []))
    );
  }

  getSubs(id): Observable<NoticeGroup[]> {
    return this.http.get<NoticeGroup[]>(`${this.dbUrl}/subs/${id}`).pipe(
      catchError(this.handleError<NoticeGroup[]>('getSubs', []))
    );
  }
  ...
  // All
  getAll(): Observable<TabularNotice[]> {
    return this.http.get<TabularNotice[]>(`${this.dbUrl}/all`).pipe(
      catchError(this.handleError<TabularNotice[]>('getAll', []))
    );
  }
  constructor(
