...
router.post("/groups", verifyAdmin, async (req, res) => {
  try {
    let results = await db.addGroup(req.body.section, req.body.name);
    res.end();
  } catch (e) {
    console.log(e);
    res.sendStatus(500);
...
router.post("/new", verifyStaff, async (req, res) => {
  try {
    ...
    if (req.body.tags) {
      results = db.tagStudents(results.insertId, req.body.tags);
    }
    res.end();
  } catch (e) {
    console.log(e);
...
router.get("/feed/:id", verifyPupil, async (req, res) => {
  try {
    let results = await db.getFeed(req.params.id);
    res.json(results);
  } catch (e) {
    console.log(e);
    res.sendStatus(500);
...
router.get("/mobile/feed", verifyMobile, async (req, res) => {
  try {
    let results = await db.getFeed(req.user.id);
    res.json(results);
  } catch (e) {
    console.log(e);
    res.sendStatus(500);
